...
import cn.internalaudit.audit.pojo.dao.impl.CommodityDao;

public class FileUploadServletshop extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		CommodityDao commodityDao = (CommodityDao)ctx.getBean("commodityDao");
		//by zhangzhengyang
...
//		Department2Dao cc=new Department2Dao();
		Commodity commodity= commodityDao.findDataCommodityById(Long.valueOf(ID));
		
		
       System.out.println("-----------------------------------------------------------"+commodity.getDoorstylesname());
		String firstFileName = "";
		// 获得容器中上传文件夹所在的物理路径
		String savePath = this.getServletConfig().getServletContext() .getRealPath("/") + "home\\resource\\commodity\\photo\\" + newfileName + "\\";
			...
			while (it.hasNext()) {
				...
				if (obit instanceof DiskFileItem) {
					...
					if (fileName != null) {
                       ...
                       String   endurl= "home\\resource\\commodity\\photo\\"+newfileName +"\\"+newfileName+ formatName;
                       System.out.println(" 最终获得的后缀路径为"+endurl);
						...
						if (new File(fileRealPath).exists()) {
							...
							if (commodity != null) {
								String updateUrl = "";
								if (!Utils.nullOrEmpty(commodity.getPhoto())) {
									updateUrl =endurl;
								} else {
									updateUrl = endurl;
								}
								commodity.setPhoto(updateUrl);
