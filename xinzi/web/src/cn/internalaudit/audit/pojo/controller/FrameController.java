...
public class FrameController {
	...
	@Resource(name = "FrameBo")
	public IFrameBo frameBo;
	...
	@RequestMapping(value = "/pages/company/frameOperate.do")
	public String frameOperate(Locale locale, Model model,
			HttpServletRequest request, HttpServletResponse response,
			Frame entity) throws Exception {
		...
		if (ADD.equals(operation)) {
			...
			frameBo.update(entity);
			writer.write("true");
		} else if (DELETE.equals(operation)) {
			frameBo.delete(entity);
			writer.write("true");
		}
		writer.flush();
		return null;
	...
	@RequestMapping(value = "/pages/company/getDataFrames.do")
	public String getDataFrames(Locale locale, Model model,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		response.setContentType("text/html;charset=UTF-8");
		response.setHeader("Cache-Control", "no-cache");
		...
		int end = start + limit;
		List<Frame> entitys = new ArrayList<Frame>();
		entitys = frameBo.findByParms(map);
