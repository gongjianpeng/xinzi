...
var checkPswResource = {
    ...
    hint: {
        ...
        wording: ["密码不能为空", "密码长度不能小于6个字符", "密码长度不能大于16个字符", "密码不能和用户名相同", "密码不能是9位以下纯数字", "密码不能包含空格", "密码不能是相同的字符组成", "密码不能由连续字符组成", "密码不能过于简单"]
    },
    url: ""
};
var checkPswCtrl = {
...
$(document).ready(function() {
    $("#password").keyup(function() {
        updatePswSghUi($("#password").val(), true, 1)
    });
    $("#password").blur(function() {
          updatePswSghUi($("#password").val(), true, 2)
				  $("#password").siblings("#psw_sgh_hint").show()
    });
    $("#password").focus(function() {
          // updatePswSghUi($("#password").val(), true, 1)
    })
});
function updatePswSghUi(A, C, D) {
    var B = $("#uin").text();
    if (C == checkPswHistory.remote && A == checkPswHistory.context && B == checkPswHistory.uin) {
			
				  $("#password").siblings("#psw_sgh_hint").show().html("&nbsp;").removeClass("onCorrect");

        return notifyUi(checkPswHistory.sgh.sgh, checkPswHistory.sgh.detail, D)
    } else {
        checkPswHistory.remote = C;
        checkPswHistory.context = A;
        checkPswHistory.uin = B;
        checkPswHistory.sgh = checkPswSgh(A, B);
				

        if (!C || 1 != checkPswHistory.sgh.sgh) {
            return notifyUi(checkPswHistory.sgh.sgh, checkPswHistory.sgh.detail, D)
				   $("#password").siblings("#psw_sgh_hint").show().html("&nbsp;").addClass("onCorrect");
        }
		      else {
           $("#password").siblings("#psw_sgh_hint").show().html("&nbsp;").addClass("onCorrect");
         }
				   
        $.getJSON(checkPswResource.url, {
            psw: A,
            uin: checkPswHistory.uin
        },
        function(E) {
            checkPswHistory.sgh = E;
            return notifyUi(E.sgh, E.detail, D)
        })
    }
}
function notifyUi(C, D, F) {
    $("#psw_sgh").attr("src", checkPswResource.sgh[C]);
    var A = $("#psw_sgh_hint");
    var E;
    var B;
    if (2 == F) {
        if (0 != C) {
            E = checkPswResource.hint.success;
            B = ""
						 $("#password").siblings("#psw_sgh_hint").show().html("&nbsp;").addClass("onCorrect");
        } else {
            E = checkPswResource.hint.fail;
            B = checkPswResource.hint.wording[D]
        }
    } else {
        if (1 == F && 0 == C && (5 == D || 2 == D)) {
            E = checkPswResource.hint.fail;
...
function checkPswSgh(psw, account) {
    if (0 == psw.length) {
        return {
            sgh: 0,
            detail: 0
        ...
        if (/ /.test(psw)) {
            ...
            if (checkPswCtrl.min_len > psw.length) {
                return {
                    sgh: 0,
                    detail: 1
                }
                ...
                if (16 < psw.length) {
                    ...
                    if (account == psw) {
                        ...
                        if (eval("/^\\d{1," + checkPswCtrl.max_purenum_len + "}$/").test(psw)) {
                            return {
                                sgh: 0,
                                detail: 4
                            }
                            ...
                            if (/^(.)\1+$/.test(psw)) {
                                ...
                                if (consectiveStr(psw)) {
                                    ...
                                    var type = 0;
                                    var charCode;
                                    for (var idx = 0; idx < psw.length; idx++) {
                                        charCode = psw.charCodeAt(idx);
