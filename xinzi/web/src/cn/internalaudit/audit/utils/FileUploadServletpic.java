...
public class FileUploadServletpic extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		IPlicalbumDao plialbumDao = (IPlicalbumDao)ctx.getBean("plicalbumDao");
	//by zhangzhengyang
	...
	//	Department2Dao cc=new Department2Dao();
     ...
     //  System.out.println("-----------------------------------------------------------"+dealer.getDename());
		...
		// 获得容器中上传文件夹所在的物理路径
		String savePath = this.getServletConfig().getServletContext() .getRealPath("/") + "picalbum\\" + newfileName + "\\";
					...
					if (fileName != null) {
                       ...
                       System.out.println(" 最终获得的后缀路径为"+endurl);
						...
						if (new File(fileRealPath).exists()) {
							...
							if (poam != null) {
								String updateUrl = "";
								if (!Utils.nullOrEmpty(poam.getFilurl())) {
									updateUrl =endurl;
								} else {
									updateUrl = endurl;
								}
