...
public class PaletteController {
	...
	@Resource(name = "PaletteBo")
	public IPaletteBo PaletteBo;
	...
	@RequestMapping(value = "/pages/company/PaletteOperate.do")
	public String PaletteController(Locale locale, Model model,
			HttpServletRequest request, HttpServletResponse response,
			Palette entity) throws Exception {
		...
		if (ADD.equals(operation)) {
			entity.setOrg(orgid);
			PaletteBo.save(entity);
			writer.write("true");
		} else if (UPDATE.equals(operation)) {
		
			PaletteBo.update(entity);
			writer.write("true");
		} else if (DELETE.equals(operation)) {
			PaletteBo.delete(entity);
			writer.write("true");
		}
		writer.flush();
		return null;
	...
	@RequestMapping(value = "/pages/company/getDataPalettes.do")
	public String getDataPalettes(Locale locale, Model model,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		response.setContentType("text/html;charset=UTF-8");
		response.setHeader("Cache-Control", "no-cache");
       ...
       if (!Utils.nullOrEmpty(orgid)){
		...
		PrintWriter writer = response.getWriter();
		DataModel datamodel = new DataModel<Palette>();
		int end = start + limit;
		List<Palette> entitys = new ArrayList<Palette>();
		entitys = PaletteBo.findByParms(map);
