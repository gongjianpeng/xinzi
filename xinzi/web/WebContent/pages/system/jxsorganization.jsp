...
$(document).ready(function() {
	function loadCombo(select,comboType) {
		...
		$.ajax( {
			type : 'GET',
			url : '../controlCriterion/getComboData.do',
	...
	$('#myTree').omTree(
		{
			...
			onSelect : function(nodedata) {
				 ...
				 if(nodedata.isHeadquarters != null){
					 $('#mytable').omGrid('setData','getjxsOrganizationByTreeId.do?id=' + nodedata.id);
					 $('#deptTable').omGrid('setData','getjxsDepartmentsByOrg.do?orgId=' + nodedata.id);
					 ...
					 $('#mytable').omGrid('setData','getjxsOrganizationByTreeId.do');
					 $('#deptTable').omGrid('setData','getjxsDepartmentsByOrg.do?deptId=' + nodedata.id);
					 ...
					 $('#mytable').omGrid('setData','getjxsOrganizationByTreeId.do');
					 $('#deptTable').omGrid('setData','getjxsDepartmentsByOrg.do');
					 $('#menu').omMenu('disableItem','001').omMenu('disableItem','002').omMenu('disableItem','003')
			...
			dataSource : "getjxsOrganizationTree.do"
		...
		buttons : [ {
			...
			click : function() {
			   if(deptvalidator.form()){
					...
					{
						url : 'jxsdepartmentSave.do',
						success : function(responseText) {
						            if(responseText=="true"){
										$.omMessageTip.show({title: $('#jsDialogSaveOk').val(), content: $('#jsDialogSaveOk').val(), timeout: 3500});
										$("#dialog-dept").omDialog("close");
										 $('#myTree').omTree('setData','getjxsOrganizationTree.do').omTree('refresh');
    ...
    var modifydepartment = function(){
		...
		showDeptDialog('部门修改',treeselectrow);
	};
...
function initDeptGrid(){
	$('#deptTable').omGrid( {
		...
		limit : 0 ,
		colModel : [ {
			header : "<fmt:message key='department.code'/>",
			name : 'code',
			width : 120,
			align : 'center'
		}, {
			header : "<fmt:message key='department.name'/>",
			name : 'name',
			width : 150,
			align : 'center'
		},{
			header : "<fmt:message key='department.address'/>",
			name : 'address',
			width : 150,
			align : 'center'
		}, {
			header :"<fmt:message key='remark'/>",
			name : 'remark',
			align : 'center',
			width : 150
		}
...
<body style="overflow: hidden" title="<fmt:message key='organization.page.title'/>">
<div id="treeDiv"
...
<ul>
	<li>
	<a href="#tab1"> <fmt:message key='organization.page.management.detail' /> </a>
	</li>
	<li>
	<a href="#tab2"> <fmt:message key='organization.page.management.children' /> </a>
	</li>
	<li>
	<a href="#tab3"> <fmt:message key='subodept' /> </a>
	</li>
</ul>
...
<table>
	<tr>
		<td><fmt:message key='organization.page.headquarters' /></td>
...
<table>
	...
	<tr>
		<td><fmt:message key='department.address' /></td>
...
<input id="jsTableAddress" value="<fmt:message key='organization.page.address'/>"
	...
	id="jsMenuAdd" value="<fmt:message key='organization.js.addOrganization'/>" style="display: none" />
<input id="jsMenuUpdate" value="<fmt:message key='organization.js.delOrganization'/>"
	style="display: none" /> <input id="jsMenuDelete"
	value="<fmt:message key='organization.js.upOrganization'/>" style="display: none" /> <input
...
<input id="jsDialogOperationFailed" value="<fmt:message key='organization.js.operationFailed'/>"
	...
	value="<fmt:message key='organization.page.adddept'/>" style="display: none" />
</body>
</html>