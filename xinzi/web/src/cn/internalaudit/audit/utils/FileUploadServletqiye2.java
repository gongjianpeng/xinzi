...
public class FileUploadServletqiye2 extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		IDealerDao dealerDao = (IDealerDao)ctx.getBean("dealerDao");
		//by zhangzhengyang
...
//		Department2Dao cc=new Department2Dao();
		Dealer dealer = dealerDao.findDataDealerById(Long.valueOf(ID));
		
       System.out.println("-----------------------------------------------------------"+dealer.getDename());
		String firstFileName = "";
		// 获得容器中上传文件夹所在的物理路径
		String savePath = this.getServletConfig().getServletContext() .getRealPath("/") + "home\\resource\\pic\\pic\\" + newfileName + "\\";
					...
					if (fileName != null) {
                       ...
                       System.out.println(" 最终获得的后缀路径为"+endurl);
						...
						if (new File(fileRealPath).exists()) {
							...
							if (dealer != null) {
								String updateUrl = "";
								if (!Utils.nullOrEmpty(dealer.getPicturebook())) {
									updateUrl =endurl;
								} else {
									updateUrl = endurl;
								}
								dealer.setPicturebook(updateUrl);
