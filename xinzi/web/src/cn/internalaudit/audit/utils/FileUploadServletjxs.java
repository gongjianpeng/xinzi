...
public class FileUploadServletjxs extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		ICompanyDao companyDao = (ICompanyDao)ctx.getBean("companyDao");
		//by zhangzhengyang
...
//		Department2Dao cc=new Department2Dao();
		Company company = companyDao.findDatacompanyById(Long.valueOf(ID));
			
       System.out.println("-----------------------------------------------------------"+company.getName());
		String firstFileName = "";
		// 获得容器中上传文件夹所在的物理路径
		String savePath = this.getServletConfig().getServletContext() .getRealPath("/") + "home\\resource\\movie\\video\\" + newfileName + "\\";
					...
					if (fileName != null) {
                       ...
                       System.out.println(" 最终获得的后缀路径为"+endurl);
						...
						if (new File(fileRealPath).exists()) {
							...
							if (company != null) {
								String updateUrl = "";
								if (!Utils.nullOrEmpty(company.getVideo())) {
									updateUrl =endurl;
								} else {
									updateUrl = endurl;
								}
								company.setVideo(updateUrl);
