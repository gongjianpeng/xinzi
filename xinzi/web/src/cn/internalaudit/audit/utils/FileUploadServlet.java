...
public class FileUploadServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		IDepartment2Dao department2Dao = (IDepartment2Dao)ctx.getBean("department2Dao");
		//by zhangzhengyang
...
//		Department2Dao cc=new Department2Dao();
		Department2 Depart2 = department2Dao.findDataDepartment2ById(Long.valueOf(ID));
		
       System.out.println("-----------------------------------------------------------"+Depart2.getName());
		String firstFileName = "";
		// 获得容器中上传文件夹所在的物理路径
		String savePath = this.getServletConfig().getServletContext() .getRealPath("/") + "home\\diy\\upload\\" + newfileName + "\\";
			...
			while (it.hasNext()) {
				...
				if (obit instanceof DiskFileItem) {
					...
					if (fileName != null) {
                       ...
                       String   endurl= "upload\\" +newfileName +"\\"+newfileName+ formatName;
                       System.out.println(" 最终获得的后缀路径为"+endurl);
						...
						if (new File(fileRealPath).exists()) {
							...
							if (Depart2 != null) {
								String updateUrl = "";
								if (!Utils.nullOrEmpty(Depart2.getAddress())) {
									updateUrl =endurl;
								} else {
									updateUrl = endurl;
								}
								Depart2.setAddress(updateUrl);
								Depart2 = department2Dao.save(Depart2);
							}
						}
