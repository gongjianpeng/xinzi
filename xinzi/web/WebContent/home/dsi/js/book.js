...
$(function(){
	 // var myName="<%=session.getAttribute("brandName")%>"; 
  	...
  	$.ajax({
		type: "POST", 
		url:"/xinzi/pages/company/getDataPlicalbum2s.do?start=0&limit=100",
		async: false,
	    dataType: "json", 
		success: function(responseText){
			if(responseText && responseText.rows)
			{
				var rows = responseText.rows;
				for(var i = 0; i < rows.length; i++)
				{
					page_content[i]='<div class="default"><img src="/xinzi/picalbum/' + rows[i].filurl + '"/></div>';
				}
