        ...
        $(document).ready(function() {
            ...
            $('#dataDictionaryTable').omGrid({
             title : '订单数据列表'+
		          ...
		          //  '<a id="modify2" href="javascript:void(0)"><img src="<%=basePath%>home/resource/images/icons/edit.gif" title="订单受理中" />&nbsp;订单受理中&nbsp;</a>'+
		            '<a id="modify3" href="javascript:void(0)"><img src="<%=basePath%>home/resource/images/icons/edit.gif" title="经销商取消订单" />&nbsp;经销商取消订单&nbsp;</a>'+
		            '<a id="modify4" href="javascript:void(0)"><img src="<%=basePath%>home/resource/images/icons/edit.gif" title="支付" />&nbsp;支付&nbsp;</a>'+
		            '<a id="modify5" href="javascript:void(0)"><img src="<%=basePath%>home/resource/images/icons/edit.gif" title="确认收货" />&nbsp;确认收货&nbsp;</a>'+
		              '<a id="modify6" href="javascript:void(0)"><img src="<%=basePath%>home/resource/images/icons/edit.gif" title="交易成功" />&nbsp;交易成功&nbsp;</a>'+
		          '<a id="delete" href="javascript:void(0)"><img src="<%=basePath%>home/resource/images/icons/edit.gif" title="删除" />&nbsp;删除&nbsp;</a>'+
		        // '<a id="modify8" href="javascript:void(0)"><img src="<%=basePath%>home/resource/images/icons/edit.gif" title="取消订单" />&nbsp;取消订单&nbsp;</a>'+
                ...
                width : '100%',
                  colModel : [ 
                              ...
                              {header : "色板", name : 'panelsname', align : 'left', width : 100},
                              {header : "压花款式", name : 'doorstylename', align : 'left', width : 100},
                               ...
                               {header : "其他款式", name : 'accessoriesename', align : 'left', width : 100},
            ...
            $('#modify3').die().live("click",function(){
	            ...
	            if(selections.length>0){
	              $.omMessageBox.confirm({
                   ...
                   onClose:function(value){
                        if(value==false)
                        	return false;
                        $('#operation').val("jxsupdate3");
		                var id=selections[0].id;
		                $.post('orderOperateqiye.do',{operation:'jxsupdate3',id:id},function(){
             ...
             $('#modify4').die().live("click",function(){
	             ...
	             if(selections.length>0){
	              $.omMessageBox.confirm({
                   ...
                   onClose:function(value){
                        if(value==false)
                        	return false;
                        $('#operation').val("jxsupdate4");
		                var id=selections[0].id;
		                $.post('orderOperateqiye.do',{operation:'jxsupdate4',id:id},function(){
		                    $('#dataDictionaryTable').omGrid('reload');
		                    $('#operation').val("");
		                    $.omMessageTip.show({title: "提示", content: "成功", timeout: 1500});
		                });
	            ...
	            $.omMessageBox.alert({title:'提示',content:'厂商暂时没有发货！'});
             			return false;
	            }
	             ...
	             if(selections.length>0){
	              $.omMessageBox.confirm({
                   ...
                   onClose:function(value){
                        if(value==false)
                        	return false;
                        $('#operation').val("jxsupdate5");
		                var id=selections[0].id;
		                $.post('orderOperateqiye.do',{operation:'jxsupdate5',id:id},function(){
		                    $('#dataDictionaryTable').omGrid('reload');
		                    $('#operation').val("");
		                    $.omMessageTip.show({title: "提示", content: "成功", timeout: 1500});
		                });
	            ...
	            if(selections.length>0){
	              $.omMessageBox.confirm({
                   ...
                   onClose:function(value){
                        if(value==false)
                        	return false;
                        $('#operation').val("jxsupdate6");
		                var id=selections[0].id;
		                $.post('orderOperateqiye.do',{operation:'jxsupdate6',id:id},function(){
		                    $('#dataDictionaryTable').omGrid('reload');
		                    $('#operation').val("");
		                    $.omMessageTip.show({title: "提示", content: "成功", timeout: 1500});
		                });
	            ...
	            if(selections.length>0){
	              $.omMessageBox.confirm({
                   ...
                   onClose:function(value){
                        if(value==false)
                        	return false;
                        $('#operation').val("jxsupdate8");
		                var id=selections[0].id;
		                $.post('orderOperateqiye.do',{operation:'jxsupdate8',id:id},function(){
		                    $('#dataDictionaryTable').omGrid('reload');
		                    $('#operation').val("");
		                    $.omMessageTip.show({title: "提示", content: "成功", timeout: 1500});
		                });
            ...
            $('#delete').die().live('click', function(e) {
                ...
                $.omMessageBox.confirm({
                   ...
                   onClose:function(value){
		                ...
		                $.post('orderOperateqiye.do',{operation:'jxsdelete',id:toDeleteRecordID},function(){
		                    $('#dataDictionaryTable').omGrid('reload');
		                    $('#operation').val("");
           ...
           $('#query').bind('click', function(e) {
               var qCode=$('#qCode').val();
                ...
                if(qCode==""&&qName===""&&qstarttime==""&&qendtime==""){ //No query criteria to , to display all data
                    $('#dataDictionaryTable').omGrid("setData", 'getDataOrderDetails.do');
                }else{ //Query, display the query data
                    $('#dataDictionaryTable').omGrid("setData", 'getDataOrderDetailsQuery.do?qcode='+encodeURI(qCode)+'&qName='+encodeURI(qName)+'&qstarttime='+encodeURI(qstarttime)+'&qendtime='+encodeURI(qendtime));
                }
...
<table border="0" cellspacing="0" cellpadding="0">
  ...
  <div>
	<tr height="30" align="left">
		...
		订单编号：</font> <input id="qCode">
		订单名称：</font> <input id="qName" >
